...
function totalExpedidoDoLote(nomeLote) {
  garantirHistorico();
  return state.historicoExpedidos.filter(e => e.lote === nomeLote).reduce((s, e) => s + e.quantidadeExpedida, 0);
}
...
function totalAlocadasNoMapa(nomeLote) {
  let total = 0;
  state.areas.forEach(area => area.ruas.forEach(rua => rua.posicoes.forEach(pos => {
    ...
    if (pos.ocupada && pos.lote === nomeLote) {
      ...
      div.innerHTML = `
        <label>
          ...
          Área: ${area.nome} | Rua: ${rua.nome} | Pos: ${p+1} | RZ: ${pos.rz} | Vol: ${pos.volume||'-'}
        </label>`;
...
window.confirmarExpedicao = function () {
  ...
  if (!lote || selecionados.length===0) { alert('Selecione ao menos uma gaylord'); return; }
  ...
  if (selecionados.length>saldo) { alert(`Quantidade selecionada maior que saldo.\nSaldo disponível: ${saldo}`); return; }
  ...
  loteObj.expedidos += detalhes.length;
  if (loteObj.total - loteObj.expedidos <= 0) loteObj.ativo = false;
