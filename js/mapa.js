...
window.excluirArea = function (areaId) {
  ...
  if (!area) return;
  if (area.ruas.some(rua => rua.posicoes.some(p => p.ocupada))) return alert('Não é possível excluir. Existem gaylords alocadas.');
...
window.renderMapa = function () {
  ...
  state.areas.forEach((area, areaIndex) => {
    ...
    area.ruas.forEach((rua, ruaIndex) => {
      ...
      rua.posicoes.forEach((posicao, posIndex) => {
        ...
        p.onclick = () => abrirModal(areaIndex, ruaIndex, posIndex);
